...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (body.action === 'delete_teacher') {
      ...
      if (!['admin', 'director', 'head_of_school', 'curriculum_coordinator', 'homeroom_teacher', 'subject_teacher'].includes(target.role) || target.school_id !== callerProfile.school_id) {
        throw new Error('Only teachers or staff in your school can be deleted')
      }
    ...
    if (!['admin', 'director', 'head_of_school', 'curriculum_coordinator', 'homeroom_teacher', 'subject_teacher'].includes(role)) {
